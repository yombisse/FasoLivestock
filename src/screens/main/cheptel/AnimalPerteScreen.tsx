...
const AnimalPerteScreen = () => {
  const navigation = useNavigation<AnimalPerteNavigationProp>();
  const route = useRoute<AnimalPerteRouteProp>();
  ...
  const [datePerte, setDatePerte] = useState<Date | undefined>(undefined);
  ...
  const validate = (): boolean => {
    ...
    if (!datePerte) errors.datePerte = 'Date de perte requise';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload: PerteRequest = {
        date_perte: datePerte!.toISOString().split('T')[0],
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
        ...
        <View style={styles.fieldContainer}>
          <AppText style={styles.label}>Date de perte *</AppText>
          <AppDateTimePicker
            value={datePerte}
            onChange={setDatePerte}
            placeholder="Sélectionner la date"
          />
          {fieldErrors.datePerte && <AppText style={styles.errorText}>{fieldErrors.datePerte}</AppText>}
        </View>

        {/* Cause */}
        <View style={styles.fieldContainer}>
          <AppText style={styles.label}>Cause de la perte</AppText>
          <AppTextInput
            style={styles.input}
            placeholder="Cause probable (vol, fugue, etc.)"
...
export default AnimalPerteScreen;
